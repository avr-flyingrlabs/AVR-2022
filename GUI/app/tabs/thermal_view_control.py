...
class ThermalView(QtWidgets.QWidget):
    def __init__(self, parent: QtWidgets.QWidget) -> None:
        ...
        upsample_factor = 4  # Increase or decrease as needed

        self.pixel_width = self.width_ / self.pixels_x  / upsample_factor
        self.pixel_height = self.height_ / self.pixels_y /upsample_factor
        ...
        self.points = [
            (ix % self.camera_x, ix // self.camera_x)
            for ix in range(self.camera_total)
        ]



        # Adjusted grid creation with higher resolution
        self.grid_x, self.grid_y = np.mgrid[
            0 : self.camera_x : (self.camera_x * upsample_factor) * 1j,
            0 : self.camera_y : (self.camera_y * upsample_factor) * 1j,
        ]
...
# Function to print the 24x32 grid
    def print_grid_as_ascii(self,float_pixels, width, height):
        

        # Make sure the list has the right number of elements (32*24 = 768)
        if len(float_pixels) != width * height:
            print(f"Error: List has {len(float_pixels)} elements, but it should have {width * height} elements.")
            return
        i = 0
        # Loop through each row (24 rows)
        for row in range(height):
            i += 1
            # Get one row of 32 numbers
            start_index = row * width
            end_index = start_index + width
            row_pixels = float_pixels[start_index:end_index]
    ...
    def update_canvas(self, pixels: List[int]) -> None:
        ...
        self.canvas.clear()
        i = 0
        for ix, row in enumerate(bicubic_rotated):
            row_str = ""
            i += 1
            for jx, pixel in enumerate(row):
                #row_str += f"{int(pixel):3} " # Convert to integer and make it 3 characters wide
                brush = QtGui.QBrush(
                    QtGui.QColor(
